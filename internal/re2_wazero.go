...
func (cm *childModule) ensureScratch(ctx context.Context, size uint32) {
	...
	// instead of stranding cm.scratchPtr on freed memory. The latter would
	// be double-freed by the finalizer once a panic here drops this module
	// (the caller's deferred endOperation is not yet registered when
	// startOperation panics, so the module never returns to the pool and is
	// GC-finalized).
	var stack [1]uint64
	...
	if newPtr == 0 {
		// wasi-libc's malloc returns 0 (it does not trap) when it cannot
		// grow linear memory past --max-memory. Fail before freeing the old
		// arena so cm keeps pointing at a valid buffer, rather than
		// committing a 0 pointer that a later allocate() would write through
		// at offset 0.
